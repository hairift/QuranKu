...
class KelolaBookmark @Inject constructor(private val repositori: RepositoriQuran) {
    suspend fun simpan(data: EntitasBookmark) = repositori.simpanBookmark(data)
    suspend fun hapus(data: EntitasBookmark) = repositori.hapusBookmark(data)
    suspend fun ubahCatatan(id: Long, judul: String, isi: String) = repositori.ubahCatatan(id, judul, isi)
    fun daftar() = repositori.bookmark()
}
